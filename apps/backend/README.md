...
This is the backend half of the `agent-playground` monorepo — see the
[repo root README](../../README.md) for the overall layout and the
[frontend](../frontend/README.md) job dashboard. Unless noted otherwise, every
command below is run from **this directory** (`apps/backend/`).
...
Run this from the **repo root** (`docker-compose.yml` lives there and builds
both `apps/backend` and `apps/frontend`):
...
[direnv](https://direnv.net/), at the repo root) loads it from the Keychain:
...
to run just the database from the compose file (from the **repo root**):
...
| `CORS_ORIGINS`           | `http://localhost:3000` | Comma-separated browser origins allowed to call the API (the frontend's URL). |
...
Dockerfile           # backend image (uv build stage -> slim runtime)
app/
  main.py            # app wiring: lifespan (clients, job store), routers, CORS
...
(`docker-compose.yml` lives at the repo root — it builds this directory and
`apps/frontend` together.)
