...
- Run the whole stack (frontend + backend + postgres): `docker compose up --build` (needs `ANTHROPIC_API_KEY` exported; run from the repo root)
- Run just the database, then the backend on the host: `docker compose up -d db` then `cd apps/backend && uv run main.py`
- Run the backend: `cd apps/backend && uv run main.py` (serves on http://127.0.0.1:8000; interactive docs at `/docs`) — needs a reachable `DATABASE_URL`
...
`[tool.pytest.ini_options]` in `apps/backend/pyproject.toml`: `asyncio_mode = "auto"`, so async
...
- `apps/backend/Dockerfile` — two stages: an `ghcr.io/astral-sh/uv:python3.14-trixie-slim` builder that resolves `uv.lock` into `/app/.venv` (`--no-dev`), and a `python:3.14-slim-trixie` runtime that copies only the venv + source. Runs as the unprivileged `agent` user, `HOST=0.0.0.0`, workspace at `/data/workspace`.
